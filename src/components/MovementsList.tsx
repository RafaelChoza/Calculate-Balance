    ...
    <>
      {moves.length === 0 ? (
        ...
        <>
          ...
          <div className="flex justify-center">
          <button 
            className="bg-fuchsia-800 m-3 font-bold p-3 text-white mx-2 rounded-2xl transform transition-all hover:scale-105"
          ...
          {selectedCategory ? (
            moves.filter((moveFiltered) => moveFiltered.category === selectedCategory).length === 0 ? (
              <p className="font-bold text-3xl text-center mt-10">No hay movimientos con esa Categoria</p>
            ) : (
              moves.filter(moveFiltered => moveFiltered.category === selectedCategory)
              .map((move) => (
